...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-obsidian">
      ...
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 70% 60% at 40% 50%, rgba(201,169,110,0.07) 0%, transparent 70%)" }}
      />
      ...
      <div className="relative z-10 w-full max-w-[1200px] mx-auto px-6 md:px-10 pt-24 pb-12 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-12 md:gap-16 items-center">
        ...
        <div>
          ...
          <h1 className="font-display leading-none mb-8">
            <span className="block text-[clamp(4rem,10vw,8.5rem)] font-light tracking-tight text-cream animate-fade-up animation-delay-400 [animation-fill-mode:both]">
              Sonali
            </span>
            <span
          ...
          <div className="flex flex-wrap gap-2 mb-6 animate-fade-up animation-delay-750 [animation-fill-mode:both]">
            {roles.map((role) => (
              <span key={role} className="font-mono text-[10px] tracking-[0.1em] uppercase bg-gold/[0.08] border border-gold/20 text-gold px-3 py-1 rounded-full">
                {role}
              </span>
            ))}
          </div>

          <p className="max-w-[480px] font-light tracking-tight text-cream text-[15px] leading-[1.8] mb-10 animate-fade-up animation-delay-900 [animation-fill-mode:both]">
          ...
          <div className="flex flex-wrap items-center gap-5 mb-12 animate-fade-up animation-delay-1050 [animation-fill-mode:both]">
            ...
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 font-mono text-xs tracking-[0.18em] uppercase font-light tracking-tight text-cream transition-colors duration-300 hover:text-gold"
            >
              Get In Touch
            </Link>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 animate-fade-up animation-delay-1200 [animation-fill-mode:both]">
            ...
            <div className="flex items-center gap-4">
              {socials.map(({ Icon, href, label }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="text-muted hover:text-gold transition-colors duration-300">
                  <Icon size={17} />
                </a>
              ))}
            </div>
        ...
        <div className="md:flex flex-col items-center gap-6 animate-slide-left animation-delay-500 [animation-fill-mode:both]">
          <div className="absolute w-80 h-80 rounded-full pointer-events-none animate-float" style={{ background: "radial-gradient(circle, rgba(201,169,110,0.12) 0%, transparent 70%)" }} />
          <div
            className="relative overflow-hidden flex-shrink-0"
            style={{ width: "300px", height: "380px", borderRadius: "160px 160px 120px 120px", border: "1px solid rgba(201,169,110,0.25)", background: "linear-gradient(160deg, #161616 0%, #111111 100%)" }}
          >
            <img src="/sonali.jpeg" alt="Sonali Srichandan" className="w-full h-full object-cover object-top" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
              <span className="font-display text-[5rem] font-light leading-none" style={{ color: "rgba(201,169,110,0.4)" }}>SS</span>
              <span className="font-mono text-[10px] tracking-[0.2em] uppercase" style={{ color: "rgba(201,169,110,0.3)" }}>Add photo →</span>
            </div>
            <div className="absolute bottom-0 left-0 right-0 h-20 pointer-events-none" style={{ background: "linear-gradient(to top, rgba(201,169,110,0.08), transparent)" }} />
          </div>
      ...
      <div className="hidden md:flex absolute bottom-10 right-10 flex-col items-center gap-2 animate-fade-in animation-delay-2000 [animation-fill-mode:both] animate-float-fast">
        <span className="font-mono text-[9px] text-muted/60 tracking-[0.25em]" style={{ writingMode: "vertical-rl" }}>SCROLL</span>
